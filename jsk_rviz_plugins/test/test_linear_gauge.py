...
"""ROS 2 port of the ROS 1 linear_gauge.test rostest (rviz crash test)."""
...
def generate_test_description():
    ...
    return LaunchDescription([
        include_sample_launch('linear_gauge_sample.launch.py'),
        launch_testing.actions.ReadyToTest(),
    ])


@unittest.skipUnless(has_display(), DISPLAY_REQUIRED_MESSAGE)
class TestLinearGauge(RvizConfigCheck, unittest.TestCase):
...
@unittest.skipUnless(has_display(), DISPLAY_REQUIRED_MESSAGE)
class TestLinearGaugeAfterShutdown(unittest.TestCase):
