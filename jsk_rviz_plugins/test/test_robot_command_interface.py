...
"""ROS 2 port of the ROS 1 robot_command_interface.test rostest (rviz crash test)."""
...
def generate_test_description():
    ...
    return LaunchDescription([
        include_sample_launch('robot_command_interface_sample.launch.py'),
        launch_testing.actions.ReadyToTest(),
    ])


@unittest.skipUnless(has_display(), DISPLAY_REQUIRED_MESSAGE)
class TestRobotCommandInterface(RvizConfigCheck, unittest.TestCase):
...
@unittest.skipUnless(has_display(), DISPLAY_REQUIRED_MESSAGE)
class TestRobotCommandInterfaceAfterShutdown(unittest.TestCase):
